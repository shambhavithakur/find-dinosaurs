function init() {
  ...
  let media350 = window.matchMedia("(min-width: 350px)");
  let media432 = window.matchMedia("(min-width: 432px)");
  let media530 = window.matchMedia("(min-width: 530px)");
  let media725 = window.matchMedia("(min-width: 725px)");

  function check350(media350) {
    if (media350.matches) {
  ...
  function check530(media530) {
    if (media530.matches) {
      parentDino2.totalPerLevel = 2;
      parentDino3.totalPerLevel = 2;
      gameModel0.cellWidth = 80;
    }
  }

  function check725(media725) {
    if (media725.matches) {
  ...
  check350(media350);
  media350.addListener(check350);
  check432(media432);
  media432.addListener(check432);
  check530(media530);
  media530.addListener(check530);
  check725(media725);
  media725.addListener(check725);
  ...
  function generateLocationsAtLevel(parentDinoNum, gameModelNum, levelNum) {
    ...
    do {
      ...
      if (
        window.matchMedia("(min-width: 530px)") &&
        parentDinoNum.length === 2 &&
        cellSelectors.length > 0
      ) {
        if (
  ...
  document.querySelector(".close-shader").addEventListener("click", function() {
    shader.classList.remove("visible");
  });
}
